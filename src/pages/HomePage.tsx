            ...
            {categories.map(({ name, icon: Icon }, index) => (
              <Link key={name} to={`/browse?category=${encodeURIComponent(name)}`}>
                <Card className={`text-center group p-6 sm:p-8 ${categoriesInView ? `animate-bounce-hard delay-${(index + 1) * 100}` : 'opacity-0'}`}>
                  ...
                  <h3 className="text-xl sm:text-2xl lg:text-3xl xl:text-4xl 2xl:text-5xl font-black text-pure-white mb-2 sm:mb-3 font-display group-hover:text-primary transition-colors duration-100">{name}</h3>
